...
const addNews = asyncHandler(async (req, res) => {
  ...
  const { title, content, url, author, category } = req.body;
  console.log("req.body", req.body);
  console.log("req.files.images", req.files.image);

  let urlImage = await imageToBase64(req.files.image.path);
  ...
  const news = await News.create({
    ...
    category,
    url,
    urlToImage: `data:${req.files.image.type};base64,` + urlImage,
    addedAt: Date.now(),
  });
